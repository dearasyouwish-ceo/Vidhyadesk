-- VidyaDesk post-reset verification. Read-only: does not modify data.
...
and tablename in ('institutes','profiles','students','batches','fee_bills','fee_payments','attendance','courses','exams','leads','expenses','payroll','notifications','activity_logs') order by tablename;

select routine_name from information_schema.routines
where routine_schema='public' and routine_name in ('vd_my_institute_id','vd_is_owner','vd_set_updated_at') order by routine_name;
...
-- Legacy application tables should return zero rows.
...
-- Auth users, SMTP/email configuration and project connectivity are intentionally not modified by this script.
