-- VidyaDesk V2 — one-time migration from the current VidyaDesk V1 schema.
...
create policy "institute bootstrap insert" on public.institutes for insert to authenticated with check (not exists(select 1 from public.profiles where role='admin'));
...
create policy "students own update" on public.students for update to authenticated using(profile_id=auth.uid()) with check(profile_id=auth.uid());
